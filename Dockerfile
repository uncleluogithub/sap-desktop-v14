...
RUN sed -i 's/deb.debian.org/mirrors.aliyun.com/g' /etc/apt/sources.list &&     apt-get update &&     apt-get install -y --no-install-recommends     xvfb x11vnc novnc websockify xfce4 xfce4-terminal     curl wget firefox-esr fonts-wqy-zenhei procps &&     apt-get clean && rm -rf /var/lib/apt/lists/*
